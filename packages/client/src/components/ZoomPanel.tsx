...
export function ZoomPanel({ zoom, onChangeZoom, onClose }: {
  ...
  const change = (value: number) => onChangeZoom(Math.max(100, Math.min(200, value)));
  return (
    <div style={styles.panel} role="group" aria-label="Manual zoom">
      ...
      <div style={styles.row}>
        <button className="btn" style={{ ...styles.step, ...(zoom === 100 ? styles.stepDisabled : {}) }} disabled={zoom === 100} onClick={() => change(zoom - 5)}>−5%</button>
      ...
      <input aria-label="Manual zoom percentage" type="range" min="100" max="200" step="5" value={zoom}
